...
model.names<-split(names(models),names(models))

pHvals <- seq(min(data$pH), max(data$pH), length.out=150)

linedata<- lapply(model.names,{.  %>%  
      models[[.]]  %>% 
      predict(.,data.frame(pH=pHvals)) %>% 
      .[1:150]})

titrlines<-lapply(model.names,{.  %>% data.frame(val=linedata[[.]],dye=.,pH=pHvals)}) %>% 
  bind_rows()
...
#
pH.index<- do.call('rbind',x=c(lapply(linedata,LowLine),lapply(linedata,HighLine)))

EfRange<-data.frame(vline=pHvals[pH.index],dye=factor(c('CL','PR')))

gainData<-
  split(names(models),names(models)) %>% 
  lapply(.,function(j){
      data.frame(gain=diff(linedata[[.]][1:150])/diff(pHvals)/1000,pH=pHvals[-1],dye=j)
    }) %>% 
  bind_rows() %>%
